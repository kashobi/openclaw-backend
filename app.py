...
class Orchestrator:
    def synthesize(self, symbol, market, congressional, insider, news):
        ...
        if score >= 6:
            v = "APPROVE"
            conf_txt = f"HIGH CONVICTION BUY SIGNAL. Market Intelligence Rating {score}/15. Multiple independent intelligence layers confirm a strong bullish setup. The weight of evidence across price action, analyst consensus, and smart money activity all points higher. Rare alignment detected across all four agents."
            watch_for = ""
            what_changes = ""
        elif score >= 3:
            v = "APPROVE"
            conf_txt = f"MODERATE BUY SIGNAL. Market Intelligence Rating {score}/15. More signals favor the upside than the downside. Not a perfect setup but the data leans bullish. Manage your position size and set a clear stop loss before entering."
            watch_for = ""
            what_changes = ""
        elif score <= -5:
            v = "PASS"
            conf_txt = f"HIGH CONVICTION AVOID. Market Intelligence Rating {score}/15. Multiple independent signals are clearly negative. The data strongly suggests avoiding this position right now and waiting for conditions to improve before reconsidering."
            ...
            v = "PASS"
            conf_txt = f"CAUTION SIGNAL. Market Intelligence Rating {score}/15. More signals are negative than positive. The risk/reward does not favor entry at current price levels. Monitor for improvement."
            ...
            conf_txt = f"MIXED SIGNALS. Market Intelligence Rating {score}/15. The intelligence agents are not in agreement on this stock right now. "
...
function buildReasonCard(r, idx){
  ...
  return `<div class="vr" id="vrc${idx}">
    <div class="vr-header" data-target="${id}" onclick="toggleReason(this)">
...
function toggleReason(hdr){
  ...
  hdr.style.background=open?'':'rgba(0,82,204,0.04)';
}
...
async function run(){
  ...
  try{
    ...
    document.getElementById('vsco').textContent='Market Intelligence Rating: '+(d.score||0)+'/15';
    ...
    const card=`<div class="sg" data-s="${d.symbol}" onclick="go('${d.symbol}')"><div class="sgtop"><div class="sgsym">${d.symbol}</div><div class="sgv ${vc}">${v}</div></div><div class="sgi">$${(d.price||0).toLocaleString()} &nbsp;|&nbsp; Rating: ${d.score}/15 &nbsp;|&nbsp; ${d.name}</div></div>`;
