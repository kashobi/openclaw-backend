...
.installBtn{position:fixed;bottom:16px;right:16px;z-index:500;background:var(--accent);color:#fff;border:none;border-radius:24px;padding:12px 20px;font-family:'Space Grotesk',sans-serif;font-size:14px;font-weight:800;cursor:pointer;box-shadow:0 6px 20px rgba(0,62,170,.4);display:none;}
...
<div class="main">
  <div>
    ...
    <div id="rpt" class="rc">
      ...
      <div class="saveRow" id="saveRow" style="display:none"><button class="saveBtn" id="saveBtn">&#9734; Save to Watchlist</button></div>
...
function renderIns(data, sellValue, bigBlock){
  ...
  var cb = data.filter(function(t){ return t.is_clevel && t.action === "A"; });
  var bc = buys.length > sells.length ? "bg" : (sells.length > buys.length ? "br" : "by");
  ...
  var rows = data.slice(0,8).map(function(t){
    var k = kindMap[t.kind] || (t.action === "A" ? kindMap.buy : (t.action === "D" ? kindMap.sell : kindMap.other));
    var cls = k.c, act = k.l;
    ...
    return "<div class=\"tr\"><span class=\""+cls+"\">"+act+"</span><span>"+esc(t.name)+"</span><span class=\"gray\">"+esc(t.title)+"</span>"+sh+val+dsc+"<span class=\"gray\">"+esc(usDate(t.date))+"</span></div>";
  }).join("");
  var sellNote = "";
  if(sellValue && sellValue > 0){
    sellNote = "<div class=\"isum\">Estimated value of recent insider selling: <b>"+fmtMoney(sellValue)+"</b>" + (bigBlock ? ". This includes one unusually large block sale. Very large blocks sometimes come from a big holder rebalancing rather than an executive, so read it in context." : "") + "</div>";
  }
  s.innerHTML = "<div class=\"ic\"><div class=\"ih\"><div class=\"it\">Insider Trades</div><div class=\"ibdg "+bc+"\">"+lbl+"</div></div><div class=\"itxt\">"+sellNote+rows+"<div class=\"iwhy\">Why it matters: executives buy their own stock for only one reason, they expect it to rise. They sell for many reasons. That is why insider buying is a stronger signal than insider selling, but a large cluster of executives selling, or a very large dollar amount, is a real caution.</div></div></div>";
}
...
function run(){
  ...
  fetch(API + "/analyze?symbol=" + encodeURIComponent(val)).then(function(r){ return r.json(); }).then(function(d){
    ...
    document.getElementById("vconf").innerHTML = alertHtml + (conf[v] || "");
